...
final class SubmitLaboratoryOrderJob implements ShouldBeUnique, ShouldQueue
{
    ...
    {
        LaboratoryOrderTransmission::query()
            ->whereKey($this->transmissionId)
            ->whereNotIn('status', [
                LaboratoryTransmissionStatus::Accepted->value,
                LaboratoryTransmissionStatus::Rejected->value,
                LaboratoryTransmissionStatus::Cancelled->value,
            ])
            ->update([
                'status' => LaboratoryTransmissionStatus::ManualReview,
                'next_attempt_at' => null,
                'error_code' => 'attempts_exhausted',
                'last_error' => mb_substr($exception?->getMessage() ?? 'Tentativas esgotadas.', 0, 2000),
            ]);
    }
}
