<x-layout.app title="Requisição de exames">
    ...
    <div class="grid gap-5 lg:grid-cols-3">
        ...
        <aside class="space-y-5">
            ...
            <section class="app-card p-5"><h2 class="font-extrabold">Integração Synclab</h2>@forelse($order->laboratoryTransmissions as $transmission)<p class="mt-3 text-sm"><strong>{{ $transmission->status->value }}</strong><br><span class="safe-wrap text-xs text-slate-500">{{ $transmission->idempotency_key }}</span></p>@empty<p class="mt-3 text-sm text-slate-500">Ainda não preparada para integração.</p>@endforelse</section>
