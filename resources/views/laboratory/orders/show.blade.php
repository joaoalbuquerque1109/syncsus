...
<x-layout.app title="Requisição de exames">
    <div class="mb-6 flex flex-wrap items-end justify-between gap-4">
        <div><a href="{{ $backUrl }}" class="text-sm font-bold text-brand-700">← Voltar ao fluxo</a><h1 class="mt-2 text-2xl font-extrabold">Requisição #{{ $order->id }}</h1><p class="mt-1 text-sm text-slate-500">{{ $order->requested_at->format('d/m/Y H:i') }} · {{ $order->origin === 'reception' ? 'Recepção' : 'Atendimento médico' }}</p></div>
