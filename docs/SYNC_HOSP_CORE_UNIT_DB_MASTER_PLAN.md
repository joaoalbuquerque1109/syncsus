...
| `Audit/.../AuditTrailQuery.php:104,110` | filtro de trilha de auditoria por paciente/atendimento | `whereHas('patient', ...)`, `whereHas('encounter', ...)` | `AuditLog` (hoje único; futuro: metade Unidade) | `Patient` (Core) / `Encounter` (Unidade) | depois do split da seção 9, a metade `audit_logs` (Unidade) que faz `whereHas('patient', ...)` ainda cruza para o Core | Resolver `patient_id`/`public_id` no Core primeiro, filtrar `audit_logs.patient_id IN (...)` depois |
