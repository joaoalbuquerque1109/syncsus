...
return new class extends Migration
{
    ...
    {
        if (DB::connection()->getName() !== 'core') {
            return;
        }

        Schema::create('unit_report_snapshots', function (Blueprint $table): void {
            $table->id();
        ...
        Schema::table('backup_verifications', function (Blueprint $table): void {
            $table->foreignId('tenant_database_id')->nullable()->after('public_id')
                ->constrained('tenant_databases')->nullOnDelete();
            $table->foreignId('health_unit_id')->nullable()->after('tenant_database_id')
                ->constrained()->nullOnDelete();
    ...
    public function down(): void
    {
        ...
        Schema::dropIfExists('unit_report_snapshots');
    }
};
